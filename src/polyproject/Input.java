...
public class Input {
	...
	public static boolean checkInput(String rawInput) {
		...
		for (int i = 0; i < rawInput.length(); i++) {
			...
			if (operative[i] != ' ' && operative[i] != '(' && operative[i] != ')' && operative[i] != '^'
					&& operative[i] != '+' && operative[i] != '-' && !(operative[i] == 'x' || operative[i] == 'X')
					&& !(operative[i] == 'y' || operative[i] == 'Y') && !(operative[i] >= '0' && operative[i] <= '9')) {
